...
- Add more prompt variations and a small prompt-tuning harness to
	quickly iterate on LLM outputs.
- Add CI that runs the offline tests and optionally runs LLM tests
	when a secret `GROQ_API_KEY` is available in CI secrets.

If you'd like, I can:
- Generate a `run_agent_demo.py` file to simplify experimentation, or
- Update `planning.md` with the explanations above so the repo is
	submission-ready.

---

Generated by your pair programmer — tell me which of the next steps
you want me to do and I’ll implement it.
